grid = []
grid.append([8,2,22,97,38,15,0,40,0,75,4,5,7,78,52,12,50,77,91,8])
grid.append([49,49,99,40,17,81,18,57,60,87,17,40,98,43,69,48,4,56,62,0])
grid.append([81,49,31,73,55,79,14,29,93,71,40,67,53,88,30,3,49,13,36,65])
grid.append([52,70,95,23,4,60,11,42,69,24,68,56,1,32,56,71,37,2,36,91])
grid.append([22,31,16,71,51,67,63,89,41,92,36,54,22,40,40,28,66,33,13,80])
grid.append([24,47,32,60,99,3,45,2,44,75,33,53,78,36,84,20,35,17,12,50])
grid.append([32,98,81,28,64,23,67,10,26,38,40,67,59,54,70,66,18,38,64,70])
grid.append([67,26,20,68,2,62,12,20,95,63,94,39,63,8,40,91,66,49,94,21])
grid.append([24,55,58,5,66,73,99,26,97,17,78,78,96,83,14,88,34,89,63,72])
grid.append([21,36,23,9,75,0,76,44,20,45,35,14,0,61,33,97,34,31,33,95])
grid.append([78,17,53,28,22,75,31,67,15,94,3,80,4,62,16,14,9,53,56,92])
grid.append([16,39,5,42,96,35,31,47,55,58,88,24,0,17,54,24,36,29,85,57])
grid.append([86,56,0,48,35,71,89,7,5,44,44,37,44,60,21,58,51,54,17,58])
grid.append([19,80,81,68,5,94,47,69,28,73,92,13,86,52,17,77,4,89,55,40])
grid.append([4,52,8,83,97,35,99,16,7,97,57,32,16,26,26,79,33,27,98,66])
grid.append([88,36,68,87,57,62,20,72,3,46,33,67,46,55,12,32,63,93,53,69])
grid.append([4,42,16,73,38,25,39,11,24,94,72,18,8,46,29,32,40,62,76,36])
grid.append([20,69,36,41,72,30,23,88,34,62,99,69,82,67,59,85,74,4,36,16])
grid.append([20,73,35,29,78,31,90,1,74,31,49,71,48,86,81,16,23,57,5,54])
grid.append([1,70,54,71,83,51,54,69,16,92,33,48,61,43,52,1,89,19,67,48])

# current greatest
max_current = {'maximus':0,'row':[]}

# sums can be left,right,up,down,diagonal
def generate_sums(grid):
  start = 0

  right_left = []
  up_down = []
  diagonal = []
  
  d1 = []
  d2 = []
  count = len(grid[0])-1
  for i in range(0,len(grid[0])):
    # right,left
    right_left.append([ grid[i][0], grid[i][1], grid[i][2], grid[i][3] ])
    #up,down
    up_down.append([ grid[0][i], grid[1][i], grid[2][i], grid[3][i] ])
    d1.append(grid[i][i])
    d2.append(grid[i][count])
    count -= 1
  diagonal.append(d1)
  diagonal.append(d2)
  return {"right_left":right_left,"up_down":up_down,"diagonal":diagonal}

# pass full results dictionary through to sum_of_rows
def gather_sums(dict):
  
  maxi = {}
  current = []
  
  for key in dict:
    # iterate over items
    maxi = sum_of_row(dict[key])
    #print(maxi)
  
  return maxi
# pass a chunk from the list of lists in here to build sum
def sum_of_row(list):
  for item in list:
    c_sum = 1
    for elem in item:
      c_sum *= elem
    if len(max_current) == 0:
      max_current['maximus'] = c_sum
      max_current['row'] = item
    elif max_current['maximus'] < c_sum:
      max_current['maximus'] = c_sum
      max_current['row'] = item
      #print(max_current)
  return (max_current)

def iterate_over_grid(grid,step_size,grid_width,grid_height):
  global max_current
  #print(len(max_current))
  lGrid = grid
  lStep = step_size
  lGWidth = grid_width
  lGHeight = grid_height
  stepped = lGWidth-1
  
  # step over the grid, starting at 0,0 -> 19,19, increment by 1, get all possible values
  for i in range(0,lGWidth):
    if i+lStep <= lGWidth:
      j = 0
      while j+lStep <= lGWidth:
        #print("start row,col:"+str(i)+":"+str(j)+" to "+str(i+3)+":"+str(j+3))
        tmp_grid = []
        tmp_grid.append([grid[i][j],   grid[i][j+1],   grid[i][j+2],   grid[i][j+3]])
        tmp_grid.append([grid[i+1][j], grid[i+1][j+1], grid[i+1][j+2], grid[i+1][j+3]])
        tmp_grid.append([grid[i+2][j], grid[i+2][j+1], grid[i+2][j+2], grid[i+2][j+3]])
        tmp_grid.append([grid[i+3][j], grid[i+3][j+1], grid[i+3][j+2], grid[i+3][j+3]])
        #print(tmp_grid)
        results = generate_sums(tmp_grid)
        #print(results)
        gather_sums(results)
        #print(max_sums)
        j+=1
  return max_current

def solve_in_one_method():
  maxi, product = 0,0
  
  # left and right
  for x in range(0,16):
    for y in range(0,19):
      product = grid[y][x] * grid[y][x+1] * grid[y][x+2] * grid[y][x+3]
      if product > maxi:
        maxi = product
  # up and down
  for x in range(0,19):
    for y in range(0,16):
      product = grid[y][x] * grid[y+1][x] * grid[y+2][x] * grid[y+3][x]
      if product > maxi:
        maxi = product
      
  # diagonal right
  for x in range(0,16):
    for y in range(0,16):
      product = grid[y][x] * grid[y+1][x+1] * grid[y+2][x+2] * grid[y+3][x+3]
      if product > maxi:
        maxi = product
  # diagonal left
  for x in range(0,16):
    for y in range(0,16):
      product = grid[y][x+3] * grid[y+1][x+2] * grid[y+2][x+1] * grid[y+3][x]
      if product > maxi:
        maxi = product
  return maxi
  
# version 1
print(iterate_over_grid(grid,4,20,20))
# version 2
print(solve_in_one_method())
