    ...
    public void updateMinMax()
    {
    	if (this.length() > 0 && this.length() < 3) {
    		if (this.length() == 1) {
    			...
    			System.out.println("condition met for single length.");
        		min = -1;
        		System.out.println("min: " + this.min());
        		
    		} else {
    			// if there are two values, compare and set accordingly
    			if (this.tail().getData() >= this.head().getData())
    			{
    				this.setMax(this.tail().getData());
    				this.setMin(this.head().getData());
    			} else {
    				this.setMax(this.head().getData());
    				this.setMin(this.tail().getData());
    			}
    		}
    	} else {
    		// start with head
        	ListItem cur = this.head();
        	System.out.println("current head.data: " + cur.getData());
